...
function compile-module {
    ...
    read platform major minor build <<<$(grep ${PLATFORM} /opt/platforms | sed -e 's/\./ /g')
    kernelver="${major}.x"
    unset VALID
    ...
    for module in $(grep -v \# all-modules); do
        modulefound="$(ls -ltr *.ko 2>/dev/null | wc -l)"
        [ "$modulefound" -le 0 ] && make -j$(nproc) -C "/opt/${PLATFORM}/build" M="/tmp/input/${module}/${kernelver}" ${PARMS} modules || echo "Module compiled, skipping" | tee -a compile.log
...
function processmodule() {
    ...
    if [ -n "$module" ]; then
        strip --strip-debug /tmp/input/${module}/${kernelver}/*.ko
        echo "Copying $(basename *.ko)"
        cp /tmp/input/${module}/${kernelver}/*.ko /output/${PLATFORM}/
    else
        while read F; do
            strip -g "${F}"
            echo "Copying $(basename ${F})"
            cp "${F}" "/output/${PLATFORM}"
...
#   while read platform kver; do
#     SRC_PATH="/opt/${platform}" echo "Compiling for ${platform}-${kver}" cd /opt/linux-${kver}/drivers while read dir; do
#       if [ -f "${dir}/Makefile" ]; then
#         echo "Driver `basename ${dir}`" grep "CONFIG_.*/.*" "${dir}/Makefile" | sed 's/.*\(CONFIG_[^)]*\).*/\1=n/g' > /tmp/env grep "CONFIG_.*\.o.*" "${dir}/Makefile" | sed
#       's/.*\(CONFIG_[^)]*\).*/\1=m/g' >> /tmp/env make -C "${SRC_PATH}" M=$(readlink -f "${dir}") clean cat /tmp/env | xargs -d '\n' make -C "${SRC_PATH}" M=$(readlink -f "${dir}") modules $@ fi
#       done < <(find -type d) DST_PATH="/output/compiled-mods/${platform}-${kver}" mkdir -p "${DST_PATH}" while read f; do strip -g "${f}" mv "${f}" "${DST_PATH}" done < <(find -name \*.ko) done
#       </opt/platforms
# }
...
*) echo "Command not recognized: $1" ;; esac
